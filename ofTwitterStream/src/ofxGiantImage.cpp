...
void ofxGiantImage::loadImage(string filePath){
    ...
    if( width < tileSize ||  height < tileSize){
        ...
        unsigned char tilepix[4*(int)img->getWidth()*(int)img->getHeight()];
        ofTexture * tile = new ofTexture();
        tile->allocate(img->getWidth(), img->getHeight(), GL_RGBA);
        tile->loadData(tilepix, img->getWidth(), img->getHeight(), GL_RGBA);
    ...
    ofPixels imgpix = img->getPixels();
    unsigned char tilepix[4*tileSize*tileSize];
    ...
    for(int y = 0; y < htiles; y++){
        for (int x = 0; x < wtiles; x++) {
            ...
            tile->allocate(tileWidth, tileHeight, GL_RGBA);
            
            // copy pixels into tilepix row by row
            for (int iy = 0; iy < tileHeight; iy++) {
                memcpy(&(tilepix[4 * (iy * tileWidth)]),
                       &(imgpix[4 * ( x1 + (y1+iy)*img->getWidth())]),
                       4 * tileSize * sizeof(unsigned char));
            }
            
            tile->loadData(tilepix, tileWidth, tileHeight, GL_RGBA);
            tiles.push_back(tile);
