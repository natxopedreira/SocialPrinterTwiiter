...
void processTweet::setImageDestinoMosaico(ofImage *img, string urlImageToPrint){
    imgDestinoMosaico = img;
    //imgDestinoMosaicoPrint = imgPrint;
    imgToPrint.loadImage(urlImageToPrint);
}
...
void processTweet::createGridPositions(int columnas, int filas){
    ...
    //int anchoP = imgToPrint.width/columnas;
    //int altoP = imgToPrint.height/filas;
    ...
    for (int rP = 0; rP < imgToPrint.height; rP+=altoPrint ) {

        for (int cP = 0; cP < imgToPrint.width; cP+=anchoPrint ) {

            dabatase.updateCellData("UPDATE posiciones SET posXprint="+ofToString(cP)+", posYprint="+ofToString(rP)+", fila="+ofToString(fil)+", col="+ofToString(col)+" WHERE idposiciones = "+ofToString(cuantas+1)+ "");
            cuantas++;
...
void processTweet::processImage(shared_ptr<ofTexture>& tex, int new_w, int new_h){
    ...
    ofPoint ptoPosicionMark(cell.at(7),cell.at(8));
    ...
    fboThumbPrint.begin();
        ...
        //imgDestinoMosaicoPrint->drawSubsection(0, 0, nuevoAnchoP, nuevoAltoP, ptoPrint.x, ptoPrint.y);
        imgToPrint.draw(ptoPrint.x, ptoPrint.y,nuevoAnchoP, nuevoAltoP);
    ...
    // dibujamos el id de la posision
    ofDrawBitmapStringHighlight(ofToString(ptoPosicionMark.x),9,19);
    printerHack.end();
